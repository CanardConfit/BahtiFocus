...
public static class LQ
{
    public static void RMatrixLQ(ref double[,] matrix, int rows, int cols, ref double[] tau)
    {
        ...
        for (var k = 0; k < minDim; ++k)
        {
            ...
            if (k < cols)
                Reflections.ApplyReflectionFromTheRight(ref matrix, tau[k], ref columnVector, k + 1, rows - 1, k, cols - 1, ref work);
        }
    }
        ...
        ref double[,] q)
    {
        ...
        for (var k = minDim - 1; k >= 0; --k)
        {
            ...
            Reflections.ApplyReflectionFromTheRight(ref q, tau[k], ref reflectionVector, 0, qRows - 1, k, cols - 1, ref work);
        }
    }

    public static void RMatrixLQUnPackL(ref double[,] sourceMatrix, int rows, int columns, ref double[,] lowerMatrix)
    {
    ...
    public static void LQDecomposition(ref double[,] matrix, int rows, int cols, ref double[] tau)
    {
        ...
        for (var k = 1; k <= minDim; ++k)
        {
            ...
            // Apply the reflection to the remaining submatrix
            if (k < cols)
                Reflections.ApplyReflectionFromTheRight(ref matrix, tau[k], ref reflectionVector, k + 1, rows, k, cols, ref work);
        }
    }
