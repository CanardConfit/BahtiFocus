...
namespace Bahtinov_grabber_autofocus
{
    public class LsqCalculator
    {
        ...
        private void CalculateBaseFunc(ref double[,] baseFunctions, ref float[] xValues, int numFunctions, int numPoints)
        {
        ...
        public float PeakPosition(float[] yValues, int estimatedPosition, int halfFitRange)
        {
            try
            {
                ...
                LeastSquares.BuildGeneralLeastSquares(ref yDouble, ref weights, ref baseFunctions, numPoints, 3, ref coefficients);
