...
public static class bdsvd
{
        ...
        int ncvt)
    {
        ...
        var flag = bidiagonalsvddecompositioninternal(ref d1, e1, n, isupper, isfractionalaccuracyrequired, ref u, 0,
                ...
                while (index9 > 1)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (num18 > 0.0 && AP.Math.Sqr(ssmin1 / num18) < num8)
                                ssmin1 = 0.0;
                        }
                        ...
                        else if (num7 == 1)
                        {
                            ...
                            e[index9 - 1] = f1;
                            if (ncvt > 0)
                                Rotations.ApplyRotationsFromTheLeft(isforward, index1 + vstart - 1, index9 + vstart - 1,
                                    vstart, n2_1, ref c1, ref s1, ref vt, ref work2);
                            if (nru > 0)
                                Rotations.ApplyRotationsFromTheRight(isforward, ustart, m2, index1 + ustart - 1,
                                    index9 + ustart - 1, ref c2, ref s2, ref u, ref work1);
                            if (ncc > 0)
                                Rotations.ApplyRotationsFromTheLeft(isforward, index1 + cstart - 1, index9 + cstart - 1,
    ...
    private static void svd2x2(double f, double g, double h, ref double ssmin, ref double ssmax)
    {
        ...
        if (num1 == 0.0)
        {
            ...
            if (val1_2 == 0.0)
                ...
                ssmax = Math.Max(val1_2, val2_1) *
                        Math.Sqrt(1.0 + AP.Math.Sqr(Math.Min(val1_2, val2_1) / Math.Max(val1_2, val2_1)));
        }
        else if (val2_1 < val1_2)
        {
            var num2 = 1.0 + num1 / val1_2;
            var num3 = (val1_2 - num1) / val1_2;
            var num4 = AP.Math.Sqr(val2_1 / val1_2);
        ...
        else
        {
            ...
            else
            {
                ...
                var num9 = 1.0 / (Math.Sqrt(1.0 + AP.Math.Sqr(num7 * num6)) +
                                  Math.Sqrt(1.0 + AP.Math.Sqr(num8 * num6)));
