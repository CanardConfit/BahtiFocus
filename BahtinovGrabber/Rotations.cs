...
public static class Rotations
{
    ...
    public static void GenerateRotation(double f, double g, ref double cs, ref double sn, ref double r) {
        ...
        else
        {
            var X1 = f;
            var X2 = g;
            r = Math.Sqrt(AP.Math.Sqr(X1) + AP.Math.Sqr(X2));
            cs = X1 / r;
            sn = X2 / r;
    ...
    private static void TestRotations() {
        ...
        {
            var num2 = 2 + AP.Math.RandomInteger(50);
            var num3 = 2 + AP.Math.RandomInteger(50);
            var isforward = AP.Math.RandomReal() > 0.5;
            var num4 = Math.Max(num2, num3);
            var a1 = new double[num2 + 1, num3 + 1];
            var a2 = new double[num2 + 1, num3 + 1];
            var a3 = new double[num2 + 1, num3 + 1];
            var a4 = new double[num2 + 1, num3 + 1];
            var c1 = new double[num2 - 1 + 1];
            var s1 = new double[num2 - 1 + 1];
            var c2 = new double[num3 - 1 + 1];
            var s2 = new double[num3 - 1 + 1];
            var work = new double[num4 + 1];
            for (var index2 = 1; index2 <= num2; ++index2) {
            ...
            for (var index4 = 1; index4 <= num2 - 1; ++index4)
            {
                var num5 = 2.0 * Math.PI * AP.Math.RandomReal();
                c1[index4] = Math.Cos(num5);
                s1[index4] = Math.Sin(num5);
            }

            for (var index5 = 1; index5 <= num3 - 1; ++index5)
            {
                var num6 = 2.0 * Math.PI * AP.Math.RandomReal();
                c2[index5] = Math.Cos(num6);
                s2[index5] = Math.Sin(num6);
            }

            ApplyRotationsFromTheLeft(isforward, 1, num2, 1, num3, ref c1, ref s1, ref a1, ref work);
            for (var index6 = 1; index6 <= num3; ++index6)
                ApplyRotationsFromTheLeft(isforward, 1, num2, index6, index6, ref c1, ref s1, ref a2, ref work);
            var val1_1 = 0.0;
            for (var index7 = 1; index7 <= num2; ++index7)
                for (var index8 = 1; index8 <= num3; ++index8)
                    val1_1 = Math.Max(val1_1, Math.Abs(a1[index7, index8] - a2[index7, index8]));
            var val2_2 = Math.Max(val1_1, val2_1);
            ApplyRotationsFromTheRight(isforward, 1, num2, 1, num3, ref c2, ref s2, ref a3, ref work);
            for (var index9 = 1; index9 <= num2; ++index9)
                ApplyRotationsFromTheRight(isforward, index9, index9, 1, num3, ref c2, ref s2, ref a4, ref work);
            var val1_2 = 0.0;
            for (var index10 = 1; index10 <= num2; ++index10)
                for (var index11 = 1; index11 <= num3; ++index11)
                    val1_2 = Math.Max(val1_2, Math.Abs(a3[index10, index11] - a4[index10, index11]));
            val2_1 = Math.Max(val1_2, val2_2);
        }

        Console.Write("TESTING ROTATIONS");
        Console.WriteLine();
        Console.Write("Pass count ");
        Console.Write("{0,0:d}", num1);
        Console.WriteLine();
        Console.Write("Error is ");
        Console.Write("{0,5:E3}", val2_1);
        Console.WriteLine();
    }
}