...
public static class Spline3
{
    ...
    public static void BuildAkimaSpline(double[] x, double[] y, int n, ref double[] c) {
        ...
        for (var index = 2; index <= n - 3; ++index)
            d[index] = System.Math.Abs(numArray4[index - 1]) + System.Math.Abs(numArray4[index + 1]) == 0.0
    ...
    public static double SplineInterpolation(ref double[] c, double x) {
        var num1 = (int)System.Math.Round(c[2]);
        var index1 = 3;
        var num2 = 3 + num1 - 2 + 1;
        while (index1 != num2 - 1) {
    ...
    public static void SplineLinTransX(ref double[] c, double a, double b) {
        ...
        var ds = 0.0;
        var d2s = 0.0;
        var n = (int)System.Math.Round(c[2]);
        if (a == 0.0) {
    ...
    public static void SplineLinTransY(ref double[] c, double a, double b) {
        var num = (int)System.Math.Round(c[2]);
    ...
    public static double SplineIntegration(ref double[] c, double x) {
        ...
        for (var index3 = 3; index3 <= index1 - 1; ++index3) {
            ...
            num3 = num3 + c[index4] * X + c[index4 + 1] * Math.Sqr(X) / 2.0 + c[index4 + 2] * Math.Sqr(X) * X / 3.0 +
                   c[index4 + 3] * Math.Sqr(Math.Sqr(X)) / 4.0;
        }

        var X1 = x - c[index1];
        var index5 = 3 + num1 + 4 * (index1 - 3);
        return num3 + c[index5] * X1 + c[index5 + 1] * Math.Sqr(X1) / 2.0 + c[index5 + 2] * Math.Sqr(X1) * X1 / 3.0 +
               c[index5 + 3] * Math.Sqr(Math.Sqr(X1)) / 4.0;
    }
        ...
        double f2)
    {
        ...
        x1 -= x0;
        x2 -= x0;
        var num1 = (f2 - f0 - x2 / x1 * (f1 - f0)) / (Math.Sqr(x2) - x1 * x2);
        var num2 = (f1 - f0 - num1 * Math.Sqr(x1)) / x1;
